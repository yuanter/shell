...
check_statics(){
    if [ ! -d "${filePath}/flycloud/statics" ]; then
      ...
      case $is_statics_file in
          1) 	echo "国内模式下载中。。。"
              wget -O ${filePath}/flycloud/statics.tar.gz  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/statics.tar.gz
          ;;
          2) 	echo "国外模式下载中。。。"
...
start_flycloud(){
        ...
        #启动容器
        if  [ $num -eq 1 ];then
            docker run -d --privileged=true --restart=always  --name flycloud  --tmpfs /run --tmpfs /run/lock --ulimit core=0 -p 1170:1170 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud --link redis:redis yuanter/flycloud
            echo -e "${yellow}使用关联redis模式启动成功${plain}"
        else if [ $num -eq 2 ];then
            docker run -d --privileged=true --restart=always  --name flycloud  --tmpfs /run --tmpfs /run/lock --ulimit core=0 -p 1170:1170 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud yuanter/flycloud
    ...
    if [ ! -f "${filePath}/flycloud/application.yml" ]; then
        ...
        case $is_speed_yml_file in
            1) 	echo "国内模式下载配置文件application.yml中。。。"
                wget -O $filePath/flycloud/application.yml  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/application.yml >/dev/null 2>&1
        ...
        # 配置密码
        echo -e "${yellow}输入本程序连接redis的密码: ${plain}"
...
check_install() {
    ...
    if [ ! -f "${filePath}/flycloud/app.jar" ]; then
       ...
       case $is_jar_file in
            1) 	echo "国内模式下载中。。。"
                wget -O ${filePath}/flycloud/app.jar --timeout=30 --tries=5 --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/app.jar || wget -O ${filePath}/flycloud/app.jar --timeout=30 --tries=5 --no-check-certificate http://ghb.jdmk.xyz:1888/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/app.jar
            ;;
            2) 	echo "国外模式下载中。。。"
...
check_update() {
  new_version=$(curl -s "http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/version")
  ...
  if [ -d "${filePath}/flycloud" ]; then
    ...
    if version_gt "${new_version}" "${old_version}"; then
      ...
      update_soft
      #成功后下载version文件到本地
      wget -O ${filePath}/flycloud/version  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/version  >/dev/null 2>&1
    else
     #检测旧版的jd_cookie是否还在运行，需关闭
     check_jd_cookie
     #检测是否已经下载静态文件
