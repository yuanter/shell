...
start_flycloud(){
        ...
        #启动容器
        if  [ $num -eq 1 ];then
            docker run -d --privileged=true --restart=always  --name flycloud --ulimit core=0 --network=host -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud --link redis:redis yuanter/flycloud
            echo -e "${yellow}使用--link模式启动成功${plain}"
        else if [ $num -eq 2 ];then
            docker run -d --privileged=true --restart=always  --name flycloud --ulimit core=0 --network=host -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud yuanter/flycloud
