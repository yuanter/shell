...
if ! command dmidecode --version >/dev/null 2>&1;  then
    ...
    if [[ x"${release}" == x"centos" ]]; then
       ...
       apt-get -y install dmidecode  >/dev/null 2>&1
        ...
        #删除脚本
        if [ -f "$filePath/flycloud_beta.sh" ]; then
            rm -rf $filePath/flycloud_beta.sh
        fi
        exit 1
    fi
fi
...
check_statics(){
    if [ ! -d "${filePath}/flycloud_beta/statics" ]; then
      ...
      case $is_statics_file in
          1) 	echo "国内模式下载中。。。"
              wget -O ${filePath}/flycloud_beta/statics.tar.gz  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/statics.tar.gz
          ;;
          2) 	echo "国外模式下载中。。。"
      ...
      case $input in
            ...
            1)	echo -e "${yellow}正在拉取安装redis脚本${plain}";
                ...
                case $is_speed_two in
                    1) 	echo "国内模式下载安装脚本中。。。"
                        wget -O redis_install.sh  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/redis_install.sh >/dev/null 2>&1
                        chmod +x *sh
...
start_flycloud(){
        ...
        #启动容器
        if  [ $num -eq 1 ];then
            docker run -d --privileged=true --restart=always  --name flycloud  --tmpfs /run --tmpfs /run/lock --ulimit core=0 -p 1170:1170 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud_beta:/root/flycloud --link redis:redis yuanter/flycloud:test
            echo -e "${yellow}使用--link模式启动成功${plain}"
        else if [ $num -eq 2 ];then
            docker run -d --privileged=true --restart=always  --name flycloud  --tmpfs /run --tmpfs /run/lock --ulimit core=0 -p 1170:1170 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud_beta:/root/flycloud yuanter/flycloud:test
...
check_yml(){
    ...
    if [ ! -f "${filePath}/flycloud_beta/application.yml" ]; then
        ...
        case $is_speed_yml_file in
            1) 	echo "国内模式下载配置文件application.yml中。。。"
                wget -O $filePath/flycloud_beta/application.yml  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/application.yml >/dev/null 2>&1
                echo -e "${yellow}当前新下载的application.yml文件所在路径为：$filePath/flycloud_beta${plain}"
            ;;
            2) 	echo "国外模式下载配置文件application.yml中。。。"
                wget -O $filePath/flycloud_beta/application.yml  --no-check-certificate https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/application.yml >/dev/null 2>&1
                echo -e "${yellow}当前新下载的application.yml文件所在路径为：$filePath/flycloud_beta${plain}"
            ;;
        esac

        #跳转至application.yml文件夹下
        cd $filePath/flycloud_beta
        echo -e "application.yml文件所在路径为：$filePath/flycloud_beta"
        ...
        case $host in
            ...
            1)	echo -e "${yellow}host使用默认redis${plain}";
                grep -rnl 'host:'  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/host:.*$/host: redis/g" >/dev/null 2>&1
                echo -e "\n";;
            2)	echo -e "${yellow}host使用内网或者公网ip亦或者域名（当使用公网时，请放行redis使用的公网端口）${plain}"; echo -e "\n"
                ...
                grep -rnl 'host:'  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/host:.*$/host: $url/g" >/dev/null 2>&1
            ;;
        esac
        # 配置密码
        echo -e "${yellow}设置redis的密码: ${plain}"
        read -r -p "请输入你之前设置的redis密码（必填）：" password
        grep -rnl 'password:'  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/password:.*$/password: $password/g" >/dev/null 2>&1
        # 配置端口
        ...
        grep -rnl 'port: '  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/port: [^port: 1170].*$/port: $port/g" >/dev/null 2>&1
        # 配置卡密
        echo -e "${yellow}设置授权token: ${plain}"
        read -r -p "请输入您的授权码：" token
        grep -rnl 'token:'  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/token:.*$/token: $token/g" >/dev/null 2>&1
        # 授权地址
        #echo -e "${yellow}设置授权网址: ${plain}"
        #read -r -p "请输入您的授权网址：" url
        #grep -rnl 'url:'  $filePath/flycloud_beta/application.yml | xargs sed -i -r "s/url:.*$/url: $url/g" >/dev/null 2>&1
    fi
}
...
check_install() {
    ...
    if [ ! -f "${filePath}/flycloud_beta/app.jar" ]; then
       ...
       case $is_jar_file in
            1) 	echo "国内模式下载中。。。"
                wget -O ${filePath}/flycloud_beta/app.jar  --no-check-certificate http://131.186.22.14:1888/https://github.com/yuanter/shell/raw/main/flycloud_beta/app.jar
            ;;
            2) 	echo "国外模式下载中。。。"
...
update_soft() {
  if [ -d "${filePath}/flycloud_beta" ]; then
    ...
    case $is_new_jar_file in
        1) 	echo "国内模式下载中。。。"
            wget -O ${filePath}/flycloud_beta/app.jar  --no-check-certificate http://131.186.22.14:1888/https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/app.jar
        ;;
        2) 	echo "国外模式下载中。。。"
...
check_update() {
  new_version=$(curl -s "http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/version")
  echo -e "[SUCCESS] 当前最新版本为：${new_version}"
  if [ -d "${filePath}/flycloud_beta" ]; then
    cd ${filePath}/flycloud_beta || exit
    if [ ! -f ${filePath}/flycloud_beta/version ]; then
        touch ${filePath}/flycloud_beta/version
        echo "19700101" >> ${filePath}/flycloud_beta/version
    fi
    ...
    if version_gt "${new_version}" "${old_version}"; then
      ...
      update_soft
      #成功后下载version文件到本地
      wget -O ${filePath}/flycloud_beta/version  --no-check-certificate http://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud_beta/version  >/dev/null 2>&1
    else
     #检测旧版的jd_cookie是否还在运行，需关闭
     check_jd_cookie
     #检测是否已经下载静态文件
