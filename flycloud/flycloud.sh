...
start_flycloud(){
        ...
        #启动容器
        if  [ $num -eq 1 ];then
            docker run -d --privileged=true --restart=always --network=host  --name flycloud --ulimit core=0 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud --link redis:redis yuanter/flycloud
            echo -e "${yellow}使用--link模式启动成功${plain}"
        else if [ $num -eq 2 ];then
            docker run -d --privileged=true --restart=always --network=host  --name flycloud --ulimit core=0 -v /sbin/dmidecode:/sbin/dmidecode -v /dev/mem:/dev/mem  -v ${filePath}/flycloud:/root/flycloud yuanter/flycloud
...
check_install() {
    ...
    if [ ! -f "${filePath}/flycloud/app.jar" ]; then
       ...
       case $is_jar_file in
            1) 	echo "国内模式下载中。。。"
                wget -O ${filePath}/flycloud/app.jar  --no-check-certificate http://193.123.237.163:8080/https://github.com/yuanter/shell/raw/main/flycloud/app.jar
            ;;
            2) 	echo "国外模式下载中。。。"
...
update_soft() {
  if [ -d "${filePath}/flycloud" ]; then
    ...
    case $is_new_jar_file in
        1) 	echo "国内模式下载中。。。"
            wget -O ${filePath}/flycloud/app.jar  --no-check-certificate http://193.123.237.163:8080/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/app.jar
        ;;
        2) 	echo "国外模式下载中。。。"
