...
check_statics(){
    if [ ! -d "${filePath}/flycloud/statics" ]; then
      ...
      case $is_statics_file in
          1) 	echo "国内模式下载中。。。"
              wget -O ${filePath}/flycloud/statics.tar.gz  --no-check-certificate https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/statics.tar.gz
          ;;
          2) 	echo "国外模式下载中。。。"
      ...
      case $input in
            ...
            1)	echo -e "${yellow}正在拉取安装redis脚本${plain}";
                ...
                case $is_speed_two in
                    1) 	echo "国内模式下载安装脚本中。。。"
                        wget -O redis_install.sh  --no-check-certificate https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/redis_install.sh >/dev/null 2>&1
                        chmod +x *sh
    ...
    if [ ! -f "${filePath}/flycloud/application.yml" ]; then
        ...
        case $is_speed_yml_file in
            1) 	echo "国内模式下载配置文件application.yml中。。。"
                wget -O $filePath/flycloud/application.yml  --no-check-certificate https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/application.yml >/dev/null 2>&1
...
check_install() {
    ...
    if [ ! -f "${filePath}/flycloud/app.jar" ]; then
       ...
       case $is_jar_file in
            1) 	echo "国内模式下载中。。。"
                wget -O ${filePath}/flycloud/app.jar  --no-check-certificate https://hub.jasas.eu.org/https://github.com/yuanter/shell/raw/main/flycloud/app.jar
            ;;
            2) 	echo "国外模式下载中。。。"
                wget -O ${filePath}/flycloud/app.jar  --no-check-certificate https://hub.jasas.eu.org/yuanter/shell/raw/main/flycloud/app.jar
            ;;
       esac
...
update_soft() {
  if [ -d "${filePath}/flycloud" ]; then
    ...
    case $is_new_jar_file in
        1) 	echo "国内模式下载中。。。"
            wget -O ${filePath}/flycloud/app.jar  --no-check-certificate https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/app.jar
        ;;
        2) 	echo "国外模式下载中。。。"
...
check_update() {
  new_version=$(curl -s "https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/version")
  ...
  if [ -d "${filePath}/flycloud" ]; then
    ...
    if version_gt "${new_version}" "${old_version}"; then
      ...
      update_soft
      #成功后下载version文件到本地
      wget -O ${filePath}/flycloud/version  --no-check-certificate https://hub.jasas.eu.org/https://raw.githubusercontent.com/yuanter/shell/main/flycloud/version  >/dev/null 2>&1
    else
     #检测旧版的jd_cookie是否还在运行，需关闭
     check_jd_cookie
     #检测是否已经下载静态文件
